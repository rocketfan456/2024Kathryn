# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 21:23:11 2024
...
""" Comparing Leprechaun and Rockne"""
...
ispSweep    = np.array([330, 350])
thrEngine   = np.array([22240, 35584])
mrEngine    = np.array([2.3, 2.8])
flgPressure     = np.array([1, 1])   # 10 if the engine is pressure fed, 1 if the engine is pump fed
strOxEngine = ["Oxygen", "Oxygen"]
strFuelEngine = ["RP-1", "Methane"]
strEngType  = ["Cryo", "Cryo"]
flgNew      = np.array([1, 1]) # 0 if the engine exists, 1 if it doesn't
strEngName  = ["Leprechaun", "Rockne"]  # used for legend
...
legString=('Goal',strEngName[0],strEngName[1]) # initialize the list for the legend
...
legString=(strEngName[0],strEngName[1]) # initialize the list for the legend

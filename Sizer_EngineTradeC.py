...
ispEngine    = np.array([340])
mrEngine    = np.array([1.8])
flgPressure     = np.array([10])   # 10 if the engine is pressure fed, 1 if the engine is pump fed
strOxEngine = "NTO"
strFuelEngine = "MMH"
strEngType  = "NotCryo"
flgNew      = np.array([1]) # 0 if the engine exists, 1 if it doesn't
strEngName  = ["EngineC"]  # used for legend

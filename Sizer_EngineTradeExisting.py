...
ispSweep    = np.array([305,310])
thrEngine   = np.array([2224,8896])
mrEngine    = np.array([1.8, 1.8])
flgPressure     = np.array([10, 10])   # 10 if the engine is pressure fed, 1 if the engine is pump fed
strOxEngine = ["NTO", "NTO"]
strFuelEngine = ["MMH", "MMH"]
strEngType  = ["NotCryo","NotCryo"]
flgNew      = np.array([1, 1]) # 0 if the engine exists, 1 if it doesn't
strEngName  = ["ND-1", "Sorin"]  # used for legend
